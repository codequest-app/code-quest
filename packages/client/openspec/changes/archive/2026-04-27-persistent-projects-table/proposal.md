...
**新增檔案**
- `packages/server/src/db/schema-sqlite.ts` — 加 `projects` table
- `packages/server/src/db/schema-mysql.ts` — 加 `projects` table
- `packages/server/drizzle/sqlite/0001_projects.sql`（generated migration）
- `packages/server/drizzle/mysql/0001_projects.sql`（generated migration）
- `packages/server/src/services/project-store.ts`（新）
- `packages/server/src/services/__tests__/project-store.test.ts`（新）
- `packages/server/src/socket/handlers/projects.ts`（新）
- `packages/server/src/__tests__/projects.test.ts`（新 — handler integration）
- `packages/shared/src/projects/schemas.ts`（新 — Zod schemas + EVENTS）
- `packages/shared/src/projects/index.ts`（新 — re-exports）

**修改檔案**
- `packages/summoner/src/filesystem/types.ts` — `FilesystemService` 加 `exists` method
- `packages/summoner/src/filesystem/local.ts` — 實作 `exists`
- `packages/summoner/src/test/fake-filesystem.ts`（如有）— 加 `exists` mock
- `packages/server/src/socket/handlers/session/connect.ts` — session.create 時 upsert project
- `packages/server/src/container.ts` — 註冊 ProjectStore + register projects handler
- `packages/server/src/types.ts` — 加 ProjectStore 到 HandlerContext
- `packages/server/src/socket/handlers/index.ts` — 註冊 projects handler
- `packages/shared/src/index.ts` — re-export projects module
- `packages/shared/src/events.ts` — 加 EVENTS.projects
- `packages/client/src/contexts/ProjectContext.tsx` — 改為 server-backed
- `packages/client/src/contexts/__tests__/ProjectContext.test.tsx` — 改用 fake server emit projects events
