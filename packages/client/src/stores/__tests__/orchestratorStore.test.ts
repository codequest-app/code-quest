...
describe('orchestratorStore', () => {
  ...
  it('should track workers after dispatch', () => {
    ...
    const workers = [
      { id: 'w1', task: { description: 'task1', provider: 'claude' as const }, status: 'pending' as const },
      { id: 'w2', task: { description: 'task2', provider: 'gemini' as const }, status: 'pending' as const },
    ];
  ...
  it('should mark all-complete when all workers done', () => {
    ...
    const results = [
      { id: 'w1', task: { description: 'task1', provider: 'claude' as const }, status: 'complete' as const, result: 'output1' },
      { id: 'w2', task: { description: 'task2', provider: 'gemini' as const }, status: 'complete' as const, result: 'output2' },
    ];
  ...
  it('should track orchestrator status lifecycle', () => {
    ...
    expect(useOrchestratorStore.getState().getOrchestrator('orch-1')!.status).toBe('workers-running');

    useOrchestratorStore.getState().setStatus('orch-1', 'workers-complete');
    expect(useOrchestratorStore.getState().getOrchestrator('orch-1')!.status).toBe('workers-complete');
