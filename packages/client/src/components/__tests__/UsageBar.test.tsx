...
describe('UsageBar', () => {
  it('renders progress bars for each quota tier', () => {
    render(
      <UsageBar
        usage={{
  ...
  it('uses green for low utilization', () => {
    render(<UsageBar usage={{ five_hour: { utilization: 0.3 } }} />);
    const bar = screen.getByTestId('usage-bar-5hr');
    expect(bar.className).toContain('bg-success');
  });

  it('uses yellow for medium utilization', () => {
    render(<UsageBar usage={{ five_hour: { utilization: 0.6 } }} />);
    const bar = screen.getByTestId('usage-bar-5hr');
    expect(bar.className).toContain('bg-warning');
  });

  it('uses red for high utilization', () => {
    render(<UsageBar usage={{ five_hour: { utilization: 0.9 } }} />);
    const bar = screen.getByTestId('usage-bar-5hr');
  ...
  it('shows overage badge when overageStatus is active', () => {
    render(
      <UsageBar
        usage={{
          extra_usage: {
  ...
  it('does not show overage badge when overageStatus is absent', () => {
    render(
      <UsageBar
        usage={{
          extra_usage: {
  ...
  it('hides extra usage when not enabled', () => {
    render(
      <UsageBar
        usage={{
