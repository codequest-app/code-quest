...
describe('ChatHandler > mcp', () => {
  describe('reconnect_mcp_server', () => {
    it('returns error via callback when session not found', async () => {
      ...
      const result = await claude.send('reconnect_mcp_server', {
  ...
  describe('get_mcp_servers', () => {
    it('returns error via callback when session not found', async () => {
      ...
      const result = await claude.send('get_mcp_servers', { channelId: 'nonexistent' });
      expect(result).toMatchObject({ error: 'Session not found' });
    });
  });
  ...
  describe('mcp:authenticate and mcp:clear_auth', () => {
    ...
    it('mcp:authenticate returns error when no active session', async () => {
      ...
      const result = await claude.send<{ success: boolean; error?: string }>(
        'authenticate_mcp_server',
        { channelId: 'nonexistent', serverName: 'test' },
      );
  ...
  describe('submit_mcp_oauth_callback_url', () => {
    ...
    it('returns error when session not found', async () => {
      ...
      const result = await claude.send<{ success: boolean; error?: string }>(
        'submit_mcp_oauth_callback_url',
        { channelId: 'nonexistent', serverName: 'test', callbackUrl: 'https://example.com/cb' },
      );
    ...
    it('returns error for invalid callbackUrl', async () => {
      ...
      const result = await claude.send<{ success: boolean; error?: string }>(
        'submit_mcp_oauth_callback_url',
        { channelId: 'any', serverName: 'test', callbackUrl: 'not-a-url' },
      );
  ...
  describe('MCP control socket events', () => {
    it('ensure_chrome_mcp_enabled: emits connecting then connected and returns wasDisabled:true', async () => {
      ...
      claude.socket.on('state:update' as any, (p: any) => stateUpdates.push(p));
    ...
    it('ensure_chrome_mcp_enabled: returns wasDisabled:false when already connected', async () => {
      ...
      await claude.send('ensure_chrome_mcp_enabled', { channelId: 'any' });
    ...
    it('ask_debugger_help: returns ask_debugger_help_response without state mutation', async () => {
      ...
      // No state:update with debugger state should have been pushed
