...
💡 **快速參考**:
- 使用 `/project-overview` skill 快速了解專案核心概念和術語
- 使用 `/map-system` skill 了解地圖探索相關術語
- 使用 `/battle-management` skill 了解戰鬥系統相關術語
...
### Parallel World / Worktree（平行世界）

**統一術語定義**：
- **正式名稱**：Worktree（技術層面）
- **RPG 化名稱**：平行世界 (Parallel World)
- **避免使用**：時間線、Branch（容易混淆）
...
切換 Worktree = 切換平行世界
合併 Worktree = 時空融合
Branch = 分支（技術概念，與 Worktree 不同）
時空管理器 = 平行世界管理器 = Worktree 管理介面
```

**說明**：
- 每個 worktree 是一個獨立的開發環境
- 可以同時在多個平行世界工作
- RPG 化為「平行世界」概念提升沉浸感
...
- 定義：區域內可以訪問的具體地點
- 範例：酒館、商業街、森林、山脈、公會大廳
- 用途：談論具體地點時使用

**3. 模式 (Mode)**：系統狀態
- 定義：遊戲系統的運行狀態
- 範例：探索模式、戰鬥模式、Plan Mode
- 用途：談論系統狀態時使用
