...
const TEMPLATES = {
  ...
  USER_SKILL_BODY: await load(REAL, 'user-skill-body.jsonl'),
  RESULT_SUCCESS: await load(REAL, 'result-success.jsonl'),
  RESULT_ERROR: await load(REAL, 'result-error.jsonl'),
  RESULT_IS_ERROR_NO_ERRORS: await load(REAL, 'result-is-error-no-errors.jsonl'),
  ...
  // SYNTHETIC fixtures: hand-written, no real CLI capture available yet.
  // Replace with REAL when captured via /collect-cli-fixtures.
