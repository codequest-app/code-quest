...
- [x] 1.1 Add `chokidar` to `packages/summoner/package.json` (latest 3.x).
- [x] 1.2 `pnpm install`.
- [x] 1.3 Confirm chokidar types available — no extra `@types/chokidar` needed (ships with types in 3.x).

## 2. TDD — FakeWatchService

- [x] 2.1 Red: create `packages/summoner/src/test/__tests__/fake-watch-service.test.ts`:
...
- [x] 2.2 Green: implement `packages/summoner/src/test/fake-watch-service.ts`.
- [x] 2.3 Run: confirm green.

## 3. TDD — WatchService interface

- [x] 3.1 Create `packages/summoner/src/fs-watch/types.ts` with the interface + `WatchEvent` type.
- [x] 3.2 Update `packages/summoner/src/index.ts` to export `WatchService`, `WatchEvent`, `createWatchService` factory placeholder.
...
- [x] 4.1 Red: create `packages/server/src/services/__tests__/fs-git-dirty-broadcaster.test.ts` injecting `FakeWatchService` and a fake emitter. Cases:
...
- [x] 4.2 Green: implement `packages/server/src/services/fs-git-dirty-broadcaster.ts`:
...
- [x] 6.1 Red: add integration test `packages/server/src/socket/__tests__/watch-lifecycle.test.ts` using FakeSummoner:
...
- [x] 6.3 Bind `WatchService` (TYPES.WatchService) and `FsGitDirtyBroadcaster` in `packages/server/src/container.ts`.
- [x] 6.4 Run all server tests — confirm green.

## 7. Real LocalWatchService (chokidar-backed)

- [x] 7.1 Create `packages/summoner/src/fs-watch/local.ts` implementing `WatchService` with chokidar:
...
- [x] 7.3 Opt-in integration test `packages/summoner/src/__tests__/local-watch-service.test.ts` guarded by `RUN_INTEGRATION` env (like `local-filesystem-service.test.ts`): create tmpdir, subscribe, write a file, assert callback fires within 1s, unsubscribe, close.
