...
- [ ] Start dev server, open app → existing projects appear
- [ ] Open new session in unfamiliar directory → project shows in sidebar
- [ ] Close that session → project still in sidebar
- [ ] Restart dev server → project list preserved
- [ ] Open same path twice via different sessions → only one project
- [ ] AddProjectDialog: type non-existent path → see error, dialog stays open
- [ ] AddProjectDialog: type file path (not dir) → see error
- [ ] AddProjectDialog: type valid dir → project added, dialog closes
- [ ] Open two browser tabs → add project in tab A → tab B sees it appear

## Phase 11 — Documentation

- [ ] Update `packages/server/src/services/project-store.ts` JSDoc with sync rules
- [ ] Update `packages/shared/src/projects/schemas.ts` with EVENTS contract comments
- [ ] If `docs/protocol.md` enumerates events, add `projects:*` events
- [ ] Update OpenSpec spec.md if any new requirements emerged during impl

## Phase 12 — Archive change

- [ ] Run `openspec verify persistent-projects-table` (or `/opsx:verify`)
- [ ] All tasks checked
- [ ] Manual verification done
- [ ] Run `openspec archive persistent-projects-table`
