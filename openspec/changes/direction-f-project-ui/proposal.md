...
**新增檔案**
- `packages/client/src/components/TopScopeSwitcher.tsx`（新 + stories + test）
- `packages/client/src/components/RenameProjectDialog.tsx`（新 + stories + test）
- `packages/client/src/components/RemoveProjectConfirmDialog.tsx`（新 + stories + test）

**修改檔案**
- `packages/client/src/contexts/ProjectContext.tsx` — `Project` 型別加 `pinned/lastOpenedAt`；actions 加 `pin/rename/remove`
- `packages/client/src/contexts/__tests__/ProjectContext.test.tsx` — 新增 actions tests
- `packages/client/src/components/ProjectList.tsx` — Pin/Recent 分組
- `packages/client/src/components/ProjectCard.tsx` — pin star + ⋯ menu trigger
- `packages/client/src/components/ProjectContextMenu.tsx` — 加 Rename / Remove items
- `packages/client/src/components/WorkspaceLayout.tsx` — 在 top 區渲染 `TopScopeSwitcher`
- `packages/client/src/components/__tests__/...` — 對應 component tests
