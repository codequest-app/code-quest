...
- Add `@iconify/react` + `@iconify-json/material-icon-theme` to `packages/client`. Material Icon Theme is the same icon pack VS Code's most-used icon theme uses (Apache 2.0); Iconify lets us tree-shake to only the icons we map.
