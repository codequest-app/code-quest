...
- Rename `Channel.id` → `Channel.channelId` in `apps/server/src/socket/channel.ts`.
...
- Update the `fakeChannel` helper in `apps/server/src/__tests__/channel-emitter.test.ts`.
...
- **Code**: `apps/server/src/socket/channel.ts` (class), 8 handler/infrastructure files under `apps/server/src/socket/`, and 1 test file.
