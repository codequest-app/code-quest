...
- [ ] Create `apps/server/src/socket/dirty-matchers.ts` exporting `matchesFs`, `matchesGit`, `matchesOpenspec`, `IGNORE_RES`, `GIT_META_RE`.
...
- [ ] Update `apps/server/src/container.ts` to import the three predicates from `socket/dirty-matchers.ts`.
- [ ] `pnpm -F server test` green.

### 2. Extract `subscribeDirtyForSocket`
- [ ] Create `apps/server/src/socket/dirty-subscriber.ts` exporting `subscribeDirtyForSocket(socket, cwd, dirty): Unsubscribe[]` that wires the three subscriptions and emits `files:dirty` / `git:dirty` / `openspec:dirty` with the existing payload shape.
