...
`GitService` 介面已存在（`apps/summoner/src/git/types.ts`），`LocalGitService`
...
```ts
// apps/summoner/src/git/__tests__/git-service.contract.ts
...
新增到 `apps/summoner/src/git/errors.ts`（或 types.ts）：
