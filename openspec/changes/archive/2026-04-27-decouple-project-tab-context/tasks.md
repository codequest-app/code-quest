...
- [x] 3.6 移除 useSessionSync：將 tab sync 邏輯合併到 ProjectProvider 內部（待 per-project 階段）
...
- [x] 6.1 TabProvider 加 `sessions` prop + 內部 useEffect incremental diff（addTab/removeTab/replaceActiveTab）
- [x] 6.2 WorkspaceLayout ProjectTabProvider 傳 sessions prop，移除 SessionTabSync
- [x] 6.3 App.tsx 外層 TabProvider 傳 sessions prop，移除 SessionTabSync
- [x] 6.4 刪除 SessionTabSync.tsx
- [x] 6.5 清理 test helpers 中的 SessionTabSync import
- [x] 6.6 全套 unit tests 通過
...
- [x] 8.1 reload 後 tabs 保留 cwd、project list 正確還原（手動驗證）
- [x] 8.2 切 project 不 reload（ChannelProvider 持續 mounted）— 由 WorkspaceLayout test 驗證
