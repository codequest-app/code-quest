...
Audit found **two leaks** still in `packages/server` that touch `node:fs` / `node:child_process` directly against user-local resources:

1. **`packages/server/src/socket/claude/cli.ts`** — uses `spawnSync('claude', ['plugin', ...])` and `execFile` for Claude CLI plugin operations, plus `mkdtempSync` / `readFileSync` / `rmSync` against `os.tmpdir()` to bypass the CLI's stdout-pipe truncation.
2. **`packages/server/src/socket/handlers/permission.ts`** — uses `readFile` to load `originalPath` / `newPath` from `control_request/open_diff`, which the Claude CLI delivers (paths can be commit blobs, scratch files, etc., NOT necessarily inside `fsRoots`).
...
**`PluginCliService`** (`packages/summoner/src/claude/plugin-cli.ts`)
...
**`DiffFileReader`** (`packages/summoner/src/filesystem/diff-reader.ts`)
...
- `packages/server/src/socket/claude/cli.ts`: delete file. Move call sites in `slash-command-plugins` (or wherever it's used) to consume `PluginCliService` from DI.
- `packages/server/src/socket/handlers/permission.ts`: replace direct `readFile` import with injected `DiffFileReader`. The two `readFileOrEmpty` calls in `onOpenDiff` go through the service.
- DI container (`packages/server/src/container.ts`): bind both new services to their summoner-default implementations.
...
**New files:**
- `packages/summoner/src/claude/plugin-cli.ts` — `PluginCliService` interface + `LocalPluginCliService` impl
- `packages/summoner/src/filesystem/diff-reader.ts` — `DiffFileReader` interface + `LocalDiffFileReader` impl
- `packages/summoner/src/test/fake-plugin-cli-service.ts`
- `packages/summoner/src/test/fake-diff-file-reader.ts`

**Modified:**
- `packages/summoner/src/index.ts` — re-export new services
- `packages/server/src/socket/handlers/permission.ts` — inject `DiffFileReader`
- `packages/server/src/socket/handlers/<plugins-handler>.ts` — inject `PluginCliService`
- `packages/server/src/container.ts` — bind both
- `packages/server/src/types.ts` (`HandlerContext`) — add fields for both services

**Deleted:**
- `packages/server/src/socket/claude/cli.ts`

**Tests:**
- `packages/summoner/src/__tests__/claude/plugin-cli.test.ts` — local impl smoke
- `packages/summoner/src/__tests__/filesystem/diff-reader.test.ts` — local impl reads + returns `''` on missing
...
- After this lands, `grep -rn "node:fs\|node:child_process" packages/server/src` returns only `bin/server.ts` and `db/sqlite-client.ts` — both server-infra, both expected.
