...
- Migrate `emitter.on(...)`, `emitter.broadcastAll(...)`, `emitter.broadcast(...)`, `socket.emit(...)`, `socket.on(...)`, and `ch.sendRequest(...)` call sites in `apps/server` and `apps/web` to use `EVENTS.<ns>.<name>` instead of string literals.
...
- Affected files:
  ...
  - `apps/server/src/socket/**/*.ts` and test files — ~20+ files.
  - `apps/web/src/**/*.{ts,tsx}` — ~15+ files.
