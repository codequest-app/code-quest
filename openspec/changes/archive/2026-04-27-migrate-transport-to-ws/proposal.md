...
現在是最便宜的遷移時機——server 端 `ChannelEmitter` 已把 handler 層跟 transport 完全隔離（見 `apps/server/src/socket/channel-emitter.ts`），所以這次改動只觸及 transport 邊界，20+ 個 handler 全部零改動。錯過這扇窗再往下加功能，抽象會被稀釋，成本只會變高。
...
- Client 改寫 `apps/web/src/socket/client.ts` + `rpc.ts`：依 client 端 `TRANSPORT` config 選擇底層 client（原生 `WebSocket` 自寫的 `WsClient`，或既有的 `socket.io-client`）；對外 hook API 維持完全相容
...
### Code
- `apps/server/src/socket/server.ts` — transport 入口換 WsTransport
- `apps/server/src/socket/channel-emitter.ts` — `TypedSocket` 改用自家介面，不再 import socket.io 型別
- `apps/server/src/socket/channel-manager.ts` — `register(io)` 參數型別換
- `apps/server/src/socket/types.ts` — socket.io 型別替換
- `apps/server/src/socket/ws-transport.ts` — **新增**
- `apps/web/src/socket/client.ts` — 重寫
- `apps/web/src/socket/rpc.ts` — 重寫
- `packages/shared/src/transport/envelope.ts` — **新增**
- `apps/server/src/test/fake-summoner.ts`（和 client 對應檔案）— 換底
- 所有 handler files（`apps/server/src/socket/handlers/**`）— **零改動**
