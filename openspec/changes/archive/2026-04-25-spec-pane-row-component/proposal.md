...
`apps/web/src/components/SpecPane.tsx` renders each Active change row as a `<button>` (open modal) containing a nested `<span role="button">` (Archive). HTML forbids nesting interactive elements inside a `<button>`; the file already carries a `biome-ignore` comment self-acknowledging the invalid markup. Screen-reader behavior on nested interactive elements is undefined, and keyboard tab order can skip the inner control. This is an accessibility / HTML-validity bug, not a code-organization concern.
