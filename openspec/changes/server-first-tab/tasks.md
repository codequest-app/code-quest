...
- [ ] 1.1 TabContext `syncFromServer` tests (8 tests) → 改用 sessions prop rerender，expect 等價
- [ ] 1.2 TabContext `createNewTab` tests (3 tests) → 改成 emit session:launch + 等 session:created，expect 等價
- [ ] 1.3 TabContext `new tab button` test (1 test) → 同 1.2
