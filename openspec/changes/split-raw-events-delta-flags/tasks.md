...
- [ ] Boot with both flags unset — default clean mode, DB unchanged.
- [ ] Boot with `RAW_EVENTS_WRITE_DELTAS=true` only — send a message; verify `raw_deltas` has rows; verify RawEventPanel does NOT show them.
- [ ] Boot with both `true` — verify RawEventPanel shows deltas.
- [ ] Boot with `RAW_EVENTS_READ_DELTAS=true` only — verify warning logged.
